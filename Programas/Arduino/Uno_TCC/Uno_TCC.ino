...
void loop()
{
  ...
  accPitch = (atan2(acc.YAxis, sqrt(acc.XAxis * acc.XAxis + acc.ZAxis * acc.ZAxis)) * 180.0) / M_PI;
  accRoll = (atan2(acc.XAxis, sqrt(acc.YAxis * acc.YAxis + acc.ZAxis * acc.ZAxis)) * 180.0) / M_PI;
  ...
  //Serial.flush();
   Serial.print((kalPitch - 1.74+1.2), 2);
  Serial.print(F(","));
  Serial.print((kalRoll -0.4-1), 2);
  Serial.print(F(","));
